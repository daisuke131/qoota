...
RSpec.describe "Api::V1::Articles", type: :request do
  ...
  describe "POST /api/v1/articles" do
    subject { post(api_v1_articles_path, params: params, headers: @auth_token) }
  ...
  describe "PATCH /api/v1/articles/:id" do
    subject { patch(api_v1_article_path(@article.id), params: params, headers: @auth_token) }
    let(:params) { { article: { title: Faker::Lorem.sentence, created_at: Time.current } } }

    it "指定した記事のレコードが更新される" do
      login
      @article = create(:article, user_id: @current_user.id)
  ...
  describe "DELETE /api/v1/articles/:id" do
    subject { delete(api_v1_article_path(@article.id), headers: @auth_token) }
